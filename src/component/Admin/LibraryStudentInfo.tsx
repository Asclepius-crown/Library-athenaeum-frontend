...
function LibraryStudentInfo(): JSX.Element {
  ...
  const [editData, setEditData] = useState<StudentData>({} as StudentData);
  ...
  const handleAddStudent = (e: React.FormEvent) => {
    ...
    setNewStudent({ rollNo: "", name: "", department: "", yearOfStudy: 0, admissionYear: 0, email: "" });
  };

  const handleSaveEdit = () => {
    if (editingRollNo) {
      updateStudent(editingRollNo, editData as any);
      ...
      <form onSubmit={handleAddStudent} className="grid grid-cols-1 md:grid-cols-6 gap-4 mb-6">
        {["rollNo", "name", "department", "yearOfStudy", "admissionYear", "email"].map((field) => (
          <input
            ...
            value={newStudent[field as keyof StudentData]}
            onChange={(e) => setNewStudent({ ...newStudent, [field]: field.includes("Year") ? Number(e.target.value) : e.target.value })}
            ...
            {students.length === 0 ? (
              ...
              editingRollNo === s.rollNo ? (
                <tr key={s.rollNo} className="bg-yellow-100">
                  {["rollNo", "name", "department", "yearOfStudy", "admissionYear", "email"].map((field) => (
                    <td key={field}>
                      <input
                        type={field.includes("Year") ? "number" : field === "email" ? "email" : "text"}
                        value={editData[field as keyof StudentData]}
                        onChange={(e) => setEditData({ ...editData, [field]: field.includes("Year") ? Number(e.target.value) : e.target.value })}
                ...
                <tr key={s.rollNo} className="hover:bg-gray-100">
                  ...
                  <td>{s.email}</td>
                  <td>
                     <button onClick={() => { setEditingRollNo(s.rollNo); setEditData(s as any); }} className="bg-blue-500 text-white px-2 py-1 rounded mr-1">Edit</button>
