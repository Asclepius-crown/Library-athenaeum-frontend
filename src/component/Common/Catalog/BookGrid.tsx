...
interface GroupedBook extends Omit<Book, '_id'> {
  _id: string;
...
interface BookGridProps {
  books: GroupedBook[];
  onEdit: ((book: GroupedBook) => void) | null;
  onDelete: ((id: string) => void) | null;
  onBorrow: ((copyId: string) => void) | null;
  onReserve: ((book: GroupedBook) => void) | null;
  loading: boolean;
  isAdmin: boolean;
  selectedBooks: string[];
  onSelectBook: (id: string, selected: boolean) => void;
  onManageCopies: ((book: GroupedBook) => void) | null;
  onToggleFeature: ((book: GroupedBook) => void) | null;
  onToggleWishlist: ((bookId: string) => void) | null;
...
export default function BookGrid({
  ...
  const [activeBook, setActiveBook] = useState<GroupedBook | null>(null);
  ...
  const handleBookHover = (book: GroupedBook) => {
    if (window.innerWidth >= 768 && !isLocked) {
       setActiveBook(book);
    }
  };

  const handleBookClick = (book: GroupedBook) => {
